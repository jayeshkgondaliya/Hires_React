...
export const getUserProfile = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    ...
    try {
      const { data } = await FetchAPI(ViewUserProfileAPI(), "POST", {
...
export const getJobLevel = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const { data } = await FetchAPI(jobLevelGetAPI(), "GET");
      return data.Data;
    } catch (error) {
      return thunkAPI.rejectWithValue({ error: error.message });
    }
...
export const getJobPosition = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    try {
      const { data } = await FetchAPI(jobPositionGetAPI(), "GET");
      return data.Data;
    } catch (error) {
      return thunkAPI.rejectWithValue({ error: error.message });
    }
...
export const getJobPost = createAsyncThunk(
  ...
  async (_, thunkAPI) => {
    ...
    try {
      const { data } = await FetchAPI(jobDescriptionGetUserAPI(), "POST", {
        recruiter_user_id: recruiterID,
